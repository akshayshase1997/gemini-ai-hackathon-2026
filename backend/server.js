...
// Standard PORT env var used by Cloud Run, falling back to API_BACKEND_PORT or 8080
...
app.get('*', (req, res, next) => {
  ...
  res.sendFile(path.join(frontendDistPath, 'index.html'), (err) => {
    if (err) {
      // If frontend hasn't been built or we are in local dev, fall through to 404
      next();
    }
  });
});
...
const server = app.listen(PORT, API_BACKEND_HOST, () => {
  console.log(`Vertex AI Backend listening on ${API_BACKEND_HOST}:${PORT}`);
});
